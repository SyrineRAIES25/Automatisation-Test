describe('Alert Ejection', () => {
    ...
    Cypress.on('uncaught:exception', (err, runnable) => {
      ...
      it ('Alert Ejection', function () {
        ...
        cy.wait(2000)
        cy.get('input[id$="--AlertName-inner"]').eq(0).type("Ejection Alert Test 3" , {force: true})
        ...
        cy.get('input[id^="__input0-__list9"]').eq(3).type("N" , {force: true})
        ...
        cy.get('#__list6>li').each(($e,index)=>{
            if ($e.text() === "Ejection Alert Test 3") {
               cy.wrap($e).click({force: true})}
               }) 
